    ...
    for year, values in year_lists.items():
        ...
        for i in range(len(values)):
            ...
            if prev_month_value == 0:  # Hantera nolldivision
                ...
                change_factor = ((current_value - prev_month_value) / prev_month_value) * 100
            change_factors[year].append(change_factor)
    return change_factors

# Funktion for att plotta bar graph
def plot_change_factor_bar_graph(year_input, area_input, category_text, agreement_input, change_factors):
    change_factors_year = change_factors[year_input]
    months = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"]
    category_text = {'V': 'villakund', 'L': 'lagenhetskund'}.get(category_text, category_text)
    agreement_input = {'R': 'rörligt', 'F1': 'Fast 1 år', 'F3': 'Fast 3 år'}.get(agreement_input, agreement_input)
...
# Simulerad data for 2017, varden satts till 0 forutom dec
jan_2018_value = 100  # Placeholder, ersatts m samma data som jan 2018 (= FF 0.00)
year_lists = {2017: [0] * 11 + [jan_2018_value]}

category_input = input("Lägenhetskund (L) eller villakund (V)? :").upper()
year_input = int(input("Ange årtalet som ska presenteras (2018-2023): "))
area_input = int(input("Ange prisområde (1-4): "))
agreement_input = input("Ange önskat prisavtal (R/F1/F3): ").upper()

filename = 'lghpriser.csv' if category_input == 'L' else 'villapriser.csv'
...
plot_change_factor_bar_graph(year_input, area_input, category_input, agreement_input, change_factors)